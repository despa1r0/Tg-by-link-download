...
3. После извлечения появятся действия, соответствующие типу публикации:
   ...
   - загрузка видео;
   - извлечение audio;
   - GIF-файл из выбранного пользователем диапазона видео;
...
**Convert to GIF** и откроет форму с отдельными полями начала и конца,
например `00:15` и `00:25` (секунды, `MM:SS` или `HH:MM:SS`). Если источник
передал длительность видео, бот покажет её в сообщении и заголовке формы.
Если длительность не передана, бот честно отметит, что она неизвестна, не
скачивая видео только ради определения длительности.
Буквы, неверный формат и время за пределами известной длительности отклоняются
до скачивания. Если диапазон длиннее 10 секунд, бот покажет предупреждение,
как в Telegram, и продолжит
конвертацию. Если готовый GIF не помещается в лимит загрузки, бот сообщит об
этом пользователю. Входной файл ограничен `MAX_DOWNLOAD_MB` (по умолчанию 50 МБ),
а готовый GIF — лимитом
