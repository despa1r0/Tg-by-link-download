...
**Convert to GIF** и откроет форму для ввода диапазона `START-END`, например
`00:15-00:25` или `1-6` (секунды, `MM:SS` или `HH:MM:SS`). Если диапазон
длиннее 10 секунд, бот покажет предупреждение, как в Telegram, и продолжит
