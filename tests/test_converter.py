...
from bot.services.converter import _timestamp_to_seconds, convert_to_discord_gif
...
class DiscordGifTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_conversion_rejects_more_than_ten_seconds(self):
        with tempfile.TemporaryDirectory() as directory:
            source = os.path.join(directory, "clip.mp4")
            Path(source).write_bytes(b"video fixture")
            self.assertIsNone(await convert_to_discord_gif(source, "0", "11"))
